use winapi::um::winnt::PAGE_EXECUTE;
use winapi::um::memoryapi::VirtualAllocEx;
use winapi::um::processthreadsapi::CreateRemoteThread;
use winapi::um::memoryapi::WriteProcessMemory;
use winapi::um::processthreadsapi::OpenProcess;
use winapi::um::winbase::INFINITE;
use winapi::um::winnt::HANDLE;
use winapi::shared::minwindef::LPVOID;
use winapi::shared::minwindef::DWORD;
//use winapi::shared::minwindef::FALSE;
use winapi::um::handleapi::CloseHandle;
use std::{mem::transmute, ptr::null_mut};

#[no_mangle]
#[link_section = ".text"]
static buf: [u8; 318] = [];



fn main() {

    unsafe {

        let th3: HANDLE = OpenProcess(

            PROCESS_CREATE_THREAD as DWORD,
            True,
            GetCurrentProcess()

       ); 

        let prctbuf = VirtualAllocEx(

            buf.as_ptr() as *mut _,           
            buf.len(),                                
            PAGE_EXECUTE as DWORD,        
            &mut old_protect  

        ); 

        let thread_handle: HANDLE = WriteProcessMemory(
            
            th3.as_ptr() as *const _,
            buf.as_ptr() as *mut _,
            prctbuf,
            buf.len(),
            buf.len()

        );


        let tthread2: HANDLE = CreateRemoteThread(

            thread_handle.as_ptr() as *const _,
            null_mut(),
            buf.len(),
            buf.as_ptr() as *const _,
            buf.as_ptr() as *const _,
            INFINITE


        );


        if thread_handle.is_null() {
            panic!("Failed to create thread");
        }

        CloseHandle(thread_handle);


    }
    
}
